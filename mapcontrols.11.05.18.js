...
require([
	...
	function (
		Map, watchUtils, esriRequest, Collection, MapView, SceneView, Camera, Basemap, BasemapToggle, BasemapToggleVM, Search, Extent, Locator, SearchVM, Home, HomeVM, PopupTemplate,
		...
		if (/iPhone|iPad|iPod|Android|webOS|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
			var view = new MapView({
				...
				scale: this.scale, //sets the initial scale to 1:2,500,000
				center: [this.lat, this.lng], //sets the center point of view with lon/lat
				constraints: {
		...
		//layers[4] = new ArcGISDynamicLayer("https://webmaps.geology.utah.gov/arcgis/rest/services/GeolMap/Map_Footprints/MapServer/0", {id:"footprints", opacity:0.9, visible:false });				//default display is level 14-15 which equals 9-10  (4-8 & 11 errors)
    	...
    	layers[6] = new TileLayer("https://webmaps.geology.utah.gov/arcgis/rest/services/GeolMap/7_5_Quads/MapServer", {
    		id: "24k-vector",
    			opacity: 0.9,
    			visible: getVisibility("24k"),
    			minScale: 50000,
    ...
    view.on("pointer-down", function (evt) {
       $("#mapNav").addClass("hidden");
		...
		// as of 4.2 you must use .hitTest to access features with mouse-events (not working)
		...
		$('#viewDiv').on('mousemove', function (e) {
			...
			var y = e.offsetY; //jsapi does screen points from bottom left instead of top left.
		...
		*/
		...
		view.then(function () {
			...
			//.queryFeatures MUST be in the view.then  (it makes no trip to server)
			layers[4].then(function () {
				var query = layers[4].createQuery();
				query.where = "series_id = '" + this.seriesid + "'"; //"series_id = 'M-212dm'";    //or M-188
				var ftrgraphics = layers[4].queryFeatures(query).then(function (results) {
		...
		var executeIdentifyTask = function (event) {
			...
			identifyParams.layerIds = [2445,1768,1754,1748,1742,1734,1730,1716,1708,1694,1690,1685,1679,1666,1644,1627,1611,1605,1592,1584,1578,1572,1561,1552,1541,1525,1507,1491,1480,1473,1829,1837,1842,1851,1891,2363,2345,2317,2309,2300,2216,2220,2224,2228,2232,2236,2240,2244,2249,2254,2258,2263,2267,2272,2277,2282,2208,2195,2182,2163,1998]		//default is ALL if ommited, must omit as layer response time is too slow for unit descriptions.
