...
require([
    ...
    view.on("click", function (evt) {
        ...
        if ($(".unit-descs").hasClass("selected")) {
            var lyr = map.findLayerById('24k-raster');
            console.log(evt);
            if ( lyr.minScale > view.scale && lyr.maxScale < view.scale){		//layer is visible?
...
view.when(function() {
    ...
    identifyParams24k.layerIds = [1802,1817,1994,1441];
    //identifyParams24k.layerIds = [2445,1768,1754,1748,1742,1734,1730,1716,1708,1694,1690,1685,1679,1666,1644,1627,1611,1605,1592,1584,1578,1572,1561,1552,1541,1525,1507,1491,1480,1473,1829,1837,1842,1851,1891,2363,2345,2317,2309,2300,2216,2220,2224,2228,2232,2236,2240,2244,2249,2254,2258,2263,2267,2272,2277,2282,2208,2195,2182,2163,1998]		//default is ALL if ommited, must omit as layer response time is too slow for unit descriptions.
    //identifyParams.layerDefinitions  = [layerDefs[#] = "STATE_NAME='Kansas' and POP2007>25000"];		//#must correspond to the layerId you are running the SQL exp on
...
function sortOutWonkyAttributes(att) {
    ...
    if (typeof att.Composition != 'undefined') {
        att.Unit_Description = att.Composition;
    } else
    if (typeof att.Compositio != 'undefined') {
        att.Unit_Description = att.Compositio;
    }
    return att;
}
        ...
        var searchTask = function (string) {
            ...
            if (/\S/.test(string)) { //test for blank, '__', null or empty
                ...
                queryTask.execute(query).when(function (response) {
        ...
        var searchTask2 = function (string) {
            ...
            if (/\S/.test(string)) { //test for blank, '__', null or empty
                ...
                findTask.execute(findParams).when(function (response) {
                    //remove anygraphics from previous searches......
                    graphicsLayer.removeAll();
