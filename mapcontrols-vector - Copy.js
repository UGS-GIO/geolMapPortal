...
require([
	...
	function (
		...
		var handle = view.watch('zoom', function (evt) {
            ...
            layerid = $.each(map.layers.items, function(index, lyr){
                if ( document.getElementById(lyr.id) ){
                    if ( lyr.minScale > view.scale && lyr.maxScale < view.scale){
                        ...
                        (dom.byId(lyr.id).checked) ? lyr.visible = true: lyr.visible = false;
            ...
            /*
            console.log( layerid );
            $('#layersPanel > div').each(function (index, d) {
                $( this ).addClass( "greyedout" );
                var inputid = $(d).children("input").attr("id");
                console.log( $(d).children("input").attr("id") );
                if ( layerid.indexOf(inputid) ){
                    $( this ).removeClass( "greyedout" );
                }
            });
            */
            // get a list of array id's
        ...
        function swapUStopo(e){
            // console.log(e);   console.log( view.zoom );   console.log( view.map.basemap.title )
            ...
            // but since it's ugly zoomed out, we keep esri topo for zooms beyond 12.1 (MUST NOT BE 12 or mobile hangs)
                if (view.zoom >= 12.1 && view.map.basemap.title == "Topographic"){
                        //console.log("change to US TOPO");
                        view.map.basemap = setBaseMap("ustopo");
                } else if (view.zoom < 12 && view.map.basemap.title == "usTopographic"){
		...
		view.on("pointer-move", function (evt) {
            ...
            view.hitTest(event).then(getFeatures);
    ...
    //        document.getElementById("name").innerHTML = "";
		...
		view.then(function () {
            ...
            if (app.seriesid){
    			...
    			fp.then(function () {
    				...
    				var ftrgraphics = fp.queryFeatures(query).then(function (results) {
		...
		on(dom.byId("layersPanel"), "change", function (e) {
            ...
            var vlyr = map.findLayerById(e.target.id+"vector");
            if (vlyr){
		...
		function addScaleSliderControl(layer, inpt, ar) {
			...
			var rangeslider = new dojox.form.HorizontalRangeSlider({
                ...
                //    layer.set( {minScale: this.value[1]} );
                //    layer.set( {maxScale: this.value[0]} );
                //    layer.refresh();
